...
class MinimalMapWindow(QMainWindow):
    ...
    def check_page_content(self):
        ...
        self.map_view.page().runJavaScript(
            ...
            lambda cls: print(f"状态类别: {cls}")
        )
        
        # 检查安全配置
        self.map_view.page().runJavaScript(
            "typeof window._AMapSecurityConfig",
            lambda type: print(f"安全配置类型: {type}")
        )
        ...
        # 检查API脚本是否加载
        self.map_view.page().runJavaScript(
            "Array.from(document.querySelectorAll('script')).some(s => s.src && s.src.includes('amap'))",
            lambda has_amap_script: print(f"包含AMap脚本: {has_amap_script}")
        )
